...
- 自動捲動以保持選中項目可見
- 點擊資料夾項目的任何位置都會展開或收合該資料夾（不限於箭頭圖示）

**`AudioItem.tsx`**
- 單一音檔項目（單行顯示）
- 包含：星級、檔名、波形圖、頻譜圖、描述
- 處理選擇狀態視覺化（簡單的背景色或邊框）
- 確保選取背景條高度與項目本身高度完全一致
- 不顯示額外的播放狀態資訊
- 只有當前選取的音檔項目才能直接設定星級或編輯描述
- 點擊非選取音檔項目的任何位置（包括星級或描述欄位）都只會選取該項目並開始播放
...
- 可編輯描述欄位
- 只有當前選取的音檔項目才能點擊進入編輯模式
- 點擊非選取音檔項目的描述欄位只會選取該項目並開始播放
- Esc 取消，Enter 或失焦儲存
- 在編輯模式時，空白鍵和方向鍵等特殊按鍵停用全域功能，直接作用於輸入欄位
- 確保輸入焦點不因畫面更新而失去
...
interface UseKeyboardNavigationReturn {
  ...
  isEditingDescription: boolean;  // 追蹤是否正在編輯描述
  ...
  setEditingDescription: (isEditing: boolean) => void;  // 設定編輯狀態
}
```

**設計決策**: 當 `isEditingDescription` 為 true 時，全域鍵盤事件處理器會忽略空白鍵和方向鍵，讓這些按鍵直接作用於輸入欄位。
...
**設計決策**: 
- 在網頁載入時從 Backend 取得所有音檔的評分和描述資料
- 更新評分或描述時，立即在瀏覽器記憶體中更新 metadata Map
- 同步傳送更新到 Backend 儲存，但不等待回應或重新取得資料
- 這樣可以避免不必要的網路請求和畫面閃爍
...
7. **描述編輯時的特殊按鍵處理**
   - 當描述欄位處於編輯模式時，停用全域鍵盤快捷鍵
   - 空白鍵在描述欄位內插入空白字元，不觸發播放控制
   - 方向鍵在描述欄位內移動游標，不切換選取項目或收合資料夾
   - 設計理由：確保輸入體驗符合使用者預期，避免意外觸發全域功能

### 視覺化反饋

- 使用高亮背景色標示當前選中項目
- 確保選取背景條高度與項目本身高度完全一致，避免蓋到上下項目內容
- 波形圖和頻譜圖上顯示播放進度指示線（垂直線）
